...
That second floor is where this register is most often wrong. The category habit is one light grey for every line on the page, which puts a text field's border between 1.2:1 and 1.5:1 against white and fails [WCAG 1.4.11](https://www.w3.org/WAI/WCAG21/Understanding/non-text-contrast.html). Measured: Tailwind's `gray-200` is 1.24:1, its `gray-300` is 1.47:1, and shadcn's default `--border` is 1.23:1. This theme carries two line colours instead: `--sl-border` for dividers and card edges, which are decoration, and `--sl-border-control` at 3.48:1 for anything a reader has to find in order to use it.
